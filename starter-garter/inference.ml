...
let rec find_pos (ls : 'a envt) x pos : 'a =
  try
    StringMap.find x ls
  with
  | Not_found -> raise (InternalCompilerError (sprintf "Name %s not found at %s" x (string_of_sourcespan pos)))
;;
let rec subst_var_typ ((tyvar, to_typ) as sub) in_typ =
  raise (NotYetImplemented "Implement substituting a type for a type variable, within a type, here")
;;
let subst_var_scheme ((tyvar, to_typ) as sub) scheme =
  raise (NotYetImplemented "Implement substituting a type for a type variable, within a scheme, here")
;;
...
let apply_subst_scheme (subst : 'a typ subst) (scheme : 'a scheme) : 'a scheme =
  raise (NotYetImplemented "Implement applying a substitution to a scheme here")
;;
let apply_subst_env (subst : 'a typ subst) (env : 'a typ envt) : 'a typ envt =
  raise (NotYetImplemented "Implement applying a substitution to a type environment here")
;;
let apply_subst_funenv (subst : 'a typ subst) (env : 'a scheme envt) : 'a scheme envt =
  raise (NotYetImplemented "Implement applying a substitution to a scheme environment here")
;;
let apply_subst_subst (subst : 'a typ subst) (dest : 'a typ subst) : 'a typ subst =
  raise (NotYetImplemented "Implement applying a substitution to another substitution here")
;;
...
let ftv_env (e : 'a typ envt) : StringSet.t = 
  raise (NotYetImplemented "Compute the free type variables of an environment here")
;;
...
let rec unify (t1 : 'a typ) (t2 : 'a typ) (loc : sourcespan) (reasons : reason list) : 'a typ subst =
  raise (NotYetImplemented "Implement type unification")
;;     
...
let instantiate (s : 'a scheme) : 'a typ =
  raise (NotYetImplemented "Implement instantiating a type scheme here")
;;
let generalize (e : 'a typ envt) (t : 'a typ) : 'a scheme =
  raise (NotYetImplemented "Implement generalizing a type here")
;;
...
let rec infer_exp (funenv : sourcespan scheme envt) (env : sourcespan typ envt) (e : sourcespan expr) reasons
  ...
  | _ -> raise (NotYetImplemented "Finish implementing inferring types for expressions")
;;
...
let infer_decl funenv env (decl : sourcespan decl) reasons : sourcespan scheme envt * sourcespan typ * sourcespan decl =
  ...
  | DFun(name, args, scheme, body, loc) ->
     raise (NotYetImplemented "Implement inferring type schemes for declarations")
;;
let infer_group funenv env (g : sourcespan decl list) : (sourcespan scheme envt * sourcespan decl list) =
  raise (NotYetImplemented "Implement inferring type schemes for declaration groups")
;;
let infer_prog funenv env (p : sourcespan program) : sourcespan program =
  match p with
  | Program(declgroups, body, typ, tag) ->
     raise (NotYetImplemented "Implement inferrence for entire programs")
;;
